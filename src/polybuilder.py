...
def main():
    ...
    if graph_subset_start is not None:
        if graph_subset_start > graph_subset_end:
            raise ValueError(f"Invalid graph subset range: start index ({graph_subset_start}) cannot be after end index ({graph_subset_end})")
        elif graph_subset_start < 0:
            raise ValueError(f"Invalid graph subset range: start index ({graph_subset_start}) cannot be negative")
    ...
    if resume_from_path:
        ...
        # Verify checkpoint settings match specified run settings
        if checkpoint.run_settings.get("face_count") != face_count:
            raise ValueError(f"Checkpoint face_count={checkpoint.run_settings.get('face_count')} does not match specified face_count={face_count}")
        ...
        if graph_subset_start is None or graph_subset_start == checkpoint.current_graph_index:
            ...
            ok = input(f"Warning: Provided graph subset start index ({graph_subset_start}) is not the same as checkpoint index ({checkpoint.current_graph_index}). This may cause missing or duplicate solutions. Proceed? (y/n) ")
            if ok.lower() != 'y':
                exit(1)
    ...
    # Determine total graph count for progress bar
    total_graph_count = scout_g6_graph_count(g6_path, graph_subset_start, graph_subset_end)
    print(f"Total graphs to process: {total_graph_count}")

    # Graphs are loaded one at a time from disk - low memory overhead
    ...
    pbar.set_postfix(passing_graphs=0, valid_realizations=0)
    
    filtered_graphs = 0
    next_gi = graph_subset_start

    # Stream graphs one at a time from file
    for graph, gi in stream_g6_graphs(g6_path, graph_subset_start, graph_subset_end):
        ...
        if vertex_count != face_count:
            ...
            pbar.close()
            print(f"Failed to process graph. Graph {gi} contains {vertex_count} nodes, expected {face_count}. Ensure input graphs have the correct number of nodes (must equal specified face count).")
            exit(1)

        if target_face_set is not None:
            if target_face_set != scout_face_set(graph):
                pbar.update(1)
                filtered_graphs += 1
                continue

        try:
            poly = create_polyhedron_from_graph(graph)
        ...
        if show_progress_details: print(f"Graph {gi}: {len(solution_list)} complete solutions with valid dihedral solutions before deduplication check")
        ...
        graphs_with_dihedral_solutions.append(graph)
        ...
        if realized_solutions:
            ...
            graphs_with_realizations.append(graph)
            ...
            if len(asymmetric_realizations) > 0:
                graphs_with_asymmetric_realizations.append(graph)
    ...
    # Close progress bar after all graphs
    pbar.close()
